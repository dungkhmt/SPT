...
public class PeopleInsertionBasedOnPopularPoint implements OnlinePeopleInsertion {
	...
	public ServiceSequence computePeopleInsertionSequence(Vehicle taxi,
		...
		int endLocID = -1;
		int rid = sel_nod[sel_nod.length-1];
		PeopleRequest peoRq = sim.mPeopleRequest.get(Math.abs(rid));
		if (peoRq != null)
			endLocID = peoRq.deliveryLocationID;
		else{
			ParcelRequest parRq = sim.mParcelRequest.get(Math.abs(rid));
			if (parRq != null)
				endLocID = parRq.deliveryLocationID;
		}
	...
	public ItineraryServiceSequence computeItineraryPeopleInsertion(
		...
		ItineraryTravelTime I = sim.establishItineraryWithPredicablePoint(taxi,
