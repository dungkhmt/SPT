...
public class GreedyDirectPeopleDirectParcelServices implements OnlinePlanner {

	public PrintWriter log = null;
	public SimulatorBookedRequests sim = null;
	public GreedyDirectPeopleDirectParcelServices(SimulatorBookedRequests sim){
		this.sim = sim;
		this.log = sim.log;
	}
	...
	public ServiceSequence computeBestProfitsParcelInsertion(Vehicle taxi, ParcelRequest pr){
		...
		if(taxi.remainRequestIDs.size() > 0){
			if(taxi.remainRequestIDs.get(0) < 0)// first point is a delivery
		...
		for(int i1 = startIdx; i1 <= taxi.remainRequestIDs.size(); i1++){
			...
			for(int i2 = i1; i2 <= i1; i2++){// direct parcel services
				...
				if(!sim.checkDirectParcelServices(nod)) continue;
